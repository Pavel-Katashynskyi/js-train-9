...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const getSub = arr.slice(startIdx);
  return getSub.join(" ");
}
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = valuesArray.filter((value) => value > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flattenedArray.map((num) => num * num);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastNElements.reduceRight((acc, num) => acc + num, 0);

  lastNElements.forEach((num) => console.log(num));
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((value, index) => {
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((accumulator, product) => {
    if (typeof product === "object" && "price" in product) {
